...
Not every Bitcoin-merge-mined chain uses AuxPoW, however; some embed the commitment in a coinbase output instead (_`OP_RETURN` tag-based merge mining_, [covered later](#op_return-tag-based-merge-mining)).
The bulk of this post focuses on AuxPoW.

This piece is a bit of a detour from my planned treatment of Bitcoin network monitoring topics, such as the [peer observer ecosystem](https://github.com/peer-observer) and the network's important properties, but there is good reason for it.
...
More specifically, those AuxPoW chains are a side-channel into block propagation (one of those important network properties) and the [[stale blocks||Valid Bitcoin blocks that lose a "chain race" to a competing block at the same height; both met `parent_target`, but only one is built upon and enters the canonical chain. The loser is called a stale.]] those propagation delays help create.
...
Each participating chain ends up with a different proof through this one shared tree: its own slot, its own sibling hashes, its own leaf-to-root path (_side mask_, introduced in the next section) through the tree.
...
Early Namecoin AuxPoW blocks even shipped it in inconsistent byte order (some in [[standard little-endian internal byte order||Bitcoin stores 32-byte hashes in little-endian internally but conventionally displays them with bytes reversed, so the same hash appears differently in raw protocol bytes vs block explorers and RPC output.]], some reversed) without breaking validation.
...
- A _branch payload_: a [[CompactSize||A variable-length integer encoding used throughout Bitcoin serialisation for lengths and counts.]] sibling count, the sibling hashes themselves, and a [[4-byte _side mask_||The 4 bytes cap each branch at 32 sibling hashes (one bit per tree level), so the proof can describe a tree of up to 2^32 leaves. Bitcoin's transaction tree and the AuxPoW slot tree both sit well within that ceiling in practice.]] whose role differs per branch.
...
This should not be confused with _blind merge mining_ (BMM), the [[Drivechain||A Bitcoin sidechain proposal where miners enforce the sidechain's withdrawal queue and can serve as merge miners without running the sidechain node. Not activated on Bitcoin mainnet.]] ([BIP 300](https://github.com/bitcoin/bips/blob/master/bip-0300.mediawiki), [BIP 301](https://github.com/bitcoin/bips/blob/master/bip-0301.mediawiki)) concept where the miner does not run the sidechain node.
...
In `OP_RETURN` tag-based merge mining, each output is a coinbase `txout` whose `scriptPubKey` is `OP_RETURN <chain tag> <payload>`, where the tag is a short ASCII identifier and the payload's contents vary widely between chains: an aux block hash for direct PoW binding (e.g., [[RSK||A Bitcoin-pegged EVM sidechain, also known as Rootstock; see [RSKIP-92](https://ips.rootstock.io/IPs/RSKIP92.html) for the merge-mining commitment format.]], tag `RSKBLOCK:`), [[accounting metadata for hashrate delegation||Metadata identifying the CoreDAO validator a Bitcoin miner is delegating hashpower to, plus the miner's CORE reward address. CoreDAO consensus credits the parent block's PoW to the validator and pays CORE rewards to the miner. Unlike RSK or AuxPoW chains, the `CORE` tag commits to no CoreDAO block hash; it is purely delegation bookkeeping.]] (e.g., CoreDAO, tag `CORE`), an aux block hash and height tag for [[reorg-safe state notarisation||Syscoin commits `(block hash, height)` of one of its own recent blocks into Bitcoin's coinbase, choosing a reorg-safe depth (5 blocks back from tip, floored to the nearest multiple of 10) at which Syscoin considers the block finalised, so the anchored block is unlikely to be reorged away.]] (e.g., [[Syscoin||A Bitcoin-merge-mined sidechain that adds a fast-finality layer on top of its PoW; see the [v5.0 release notes](https://github.com/syscoin/syscoin/blob/master/doc/release-notes/release-notes-5.0.0.md#4-auxpow-tags) for the `sys` tag spec.]], tag `sys`), or other chain-specific data.
...
Bitcoin block [948,433](https://mempool.space/block/00000000000000000001144d36d598ad54dc664e6387ce1aae928117c44435dc) as examined above illustrates this ({@fig:mempool-space-block-948433}): alongside five `OP_RETURN` outputs (the SegWit witness commitment plus four project/protocol tags, including RSK/Syscoin merge-mining commitments and Core/[[exSat||A Bitcoin sidechain that synchronises Bitcoin block headers via coinbase metadata; see [exsat.network](https://exsat.network/) for details.]] coordination metadata), the coinbase `scriptSig` carries an AuxPoW marker that, with `merkle_size = 8`, commits to up to eight auxiliary chains:
...
`OP_RETURN` tag-based merge mining sits alongside AuxPoW as a lighter, per-chain alternative: a coinbase `OP_RETURN` output per chain rather than a shared `scriptSig` marker, with each chain free to define its own tag, payload format, and commitment semantics (e.g., PoW binding like AuxPoW, or another purpose such as delegation accounting or reorg-safe state notarisation).
